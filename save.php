...
require_once(myClass."_myclass.php");
	...
	$script="SELECT * FROM tbl_checklogs WHERE Username='$Username'";
	$query = mysqli_query($con,$script) or die(mysqli_error());
	if (mysqli_num_rows($query)>0)
	 {
		header("Location: index.php?notif=exist");
	 }
	 	...
	 	else
	 	{
			...
			$script ="INSERT into tbl_checklogs (Username, Password, ConfirmPassword, datein,Picture)
		...
		";
				$query = mysqli_query($con,$script) or die(mysqli_error());
				header("Location: index1.php?notif=save");
		}	
	}
 ?>